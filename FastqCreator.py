...
class FastqCreator(IonPlugin):
    ...
    def bam2fastq_picard(self, bam_filename_list, fastq_filename):
        print "mmmmmmFastq:"+fastq_filename;
        try:
    ...
    def launch(self, data=None):
        ...
        print url_path
        CurrentTimeStr=time.strftime('%Y%m%d%H%M')
        print "hhhhh:"+CurrentTimeStr
        ...
        for dataset in datasets_basecaller['datasets']:
            ...
            TestStem=dataset['dataset_name']
            TestStem=TestStem.replace("/","_");
            print "TestStem:"+TestStem
            ...
            # output
            dataset['fastq'] = TestStem+'_'+output_stem+'.fastq'
            #if barcodeId:
            ...
            try:
                ...
                print "kkkkk:"+dataset['fastq'];
                self.bam2fastq_picard(bam_list,dataset['fastq'])
                
                print "[ " + CurrentTimeStr + " ] " + "ppppp"
                targetdir="/results/referenceLibrary/temp/Gc"+ CurrentTimeStr
                commandstr="mkdir "+targetdir
                subprocess.call(commandstr,shell=True)

                commandstr = "cp " + dataset['fastq'] + " " + targetdir + "/" + dataset['fastq']
                #subprocess.call(commandstr,shell=True)
                print "ccccc:" + commandstr
            except:
                traceback.print_exc()
